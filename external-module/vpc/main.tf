...
variable "vpn_monitor_arn" {
    description = "VPN Monitor ARN from global module"
    type = map
}

variable "customer_gateway_ids" {
    description = "Customer gateway IDs from global module"
    type = map
}
...
module "vpn1" {
  ...
  vpn_gateway_id      = aws_vpn_gateway.vgw[0].id
  customer_gateway_id = var.customer_gateway_ids[var.region]["vpnhub-aws1-pub"]
  create_alarm        = true

  alarm_actions             = try([var.vpn_monitor_arn[var.region]], null)
  insufficient_data_actions = try([var.vpn_monitor_arn[var.region]], null)
  ok_actions                = try([var.vpn_monitor_arn[var.region]], null)
}
...
module "vpn2" {
  ...
  vpn_gateway_id      = aws_vpn_gateway.vgw[0].id
  customer_gateway_id = var.customer_gateway_ids[var.region]["vpnhub-aws2-pub"]
  create_alarm        = true
